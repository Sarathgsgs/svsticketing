...
function RolePill() {
  ...
  return <span className="pill">Role: {role}</span>
}
      ...
      <aside className="sidebar">
        ...
        <div className="nav">
          ...
          {["agent","admin"].includes(role) && (
            <NavLink to="/resolve" className={({ isActive }) => (isActive ? "active" : "")}>
              Resolve
            </NavLink>
          )}
          {role === "admin" && (
            <NavLink to="/govern" className={({ isActive }) => (isActive ? "active" : "")}>
              Govern
            </NavLink>
          )}
          {!loggedIn && (
      ...
      <div style={{ display: "grid", gridTemplateRows: "52px 1fr" }}>
        <header className="topbar">
          <div className="search">
            <input placeholder="Search tickets, KB, users..." />
          </div>
